...
df[col].to_csv('wowy/player_small'+trail+'.csv',index = False)
#df[col].to_csv('2024/player_tracking/player_small.csv',index = False)
df.to_csv('wowy/player_large'+trail+'.csv',index = False)
year = 2025
df['year'] = year
...
#shotzone
old = pd.read_csv('shotzone'+trail+'.csv')
...
master.to_csv('shotzone'+trail+'.csv',index = False)
...
newdf = update_team(2024,ps=ps,vs=False)

newdf = update_team(2024,ps=ps,vs=True)
